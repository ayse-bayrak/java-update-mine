...
//Print all dish's name that has less than 400 calories
        ...
        DishData.getAll().stream()
               ...
               // .map(p->p.getName()) // can use method references
                .map(Dish::getName)
        ...
        System.out.println("Print the three high caloric dish name");
        ...
        DishData.getAll().stream()  // WHAT I have in the stream? Dish (Rice, Other, 400... all properties)
                .filter(p-> p.getCalories()<400)
                .sorted(comparing(Dish::getCalories)) // base on the what, we import `import static java.util.Comparator.comparing;`
